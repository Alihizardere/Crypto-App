...
extension Double {
  func formattedPrice(decimalCount: Int) -> String {
    let formatSpecifier = "%.\(decimalCount)f"
    let formattedNumber = String(format: formatSpecifier, self)
    let priceComponents = formattedNumber.split(separator: ".")
